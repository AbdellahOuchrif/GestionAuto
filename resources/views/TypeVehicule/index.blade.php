    ...
    <section class="container-fluid d-flex flex-column align-items-center">
      <div class="container-fluid ml-5 mt-5 d-flex justify-content-start w-25">
        <a class="btn btn-success text-color mb-3" href="{{ URL::previous() }}" role="button">Retour</a>
            ...
            <h3 class="display-2 mt-5 mb-5">Type Vehicule</h3>
            <form class="form-inline" action="{{ route("TypeVehicule.store") }}" method="POST" id="form">
                @csrf
            ...
            <table id="myTable" class="display">
                ...
                <tbody>
                    @foreach ($types as $type)
                        <tr>
                            <td style="text-transform: uppercase;">{{ $type->type }}</td>
                            <td class="actions d-flex justify-content-end">
                                <form action="{{ route('TypeVehicule.destroy', $type->id) }}" method="POST" style="display: inline-block">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <button type="button" class="btn btn-outline-warning btn-sm" data-toggle="modal" data-target="#exampleModal" onclick="getTableId({{ $type->id }}, '{{ $type->type }}')"><span class="material-symbols-outlined">edit</span></button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Modifier Type Vehicule</h5>
