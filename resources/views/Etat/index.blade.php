    ...
    <section class="container-fluid d-flex flex-column align-items-center">
      <div class="container-fluid ml-5 mt-5 d-flex justify-content-start w-25">
        <a class="btn btn-success text-color mb-3" href="{{ URL::previous() }}" role="button">Retour</a>
            ...
            <h3 class="display-2 mt-5 mb-5">Etat Reservation</h3>
            <form class="form-inline" action="{{ route("Etat.store") }}" method="POST" id="form">
                @csrf
            ...
            <table id="myTable" class="display">
                ...
                <tbody>
                    @foreach ($etats as $etat)
                        <tr>
                            <td style="text-transform: uppercase;">{{ $etat->designation }}</td>
                            <td class="actions d-flex justify-content-end">
                                <form action="{{ route('Etat.destroy', $etat->id) }}" method="POST" style="display: inline-block">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <button type="button" class="btn btn-outline-warning btn-sm" data-toggle="modal" data-target="#exampleModal" onclick="getTableId({{ $etat->id }}, '{{ $etat->designation }}')"><span class="material-symbols-outlined">edit</span></button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
...
@push('scripts')
    ...
    <script>
        ...
        function getTableId(id, designation_etat) {
            //let modifier_etat_form = document.querySelector("#modifier_etat_form");
            //let designation = document.querySelector("#designation_etat");
            let modifier_action = "{{route('Etat.update', '')}}"+"/"+id;
            UpdateForm.action = modifier_action;
            UpdateInput.value = designation_etat;
        }
    </script> 
@endpush