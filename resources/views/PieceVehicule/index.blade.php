    ...
    <section class="container-fluid d-flex flex-column align-items-center">
      <div class="container-fluid ml-5 mt-5 d-flex justify-content-start w-25">
        <a class="btn btn-success text-color mb-3" href="{{ URL::previous() }}" role="button">Retour</a>
            ...
            <h3 class="display-2 mt-5 mb-5">Piece Vehicule</h3>
            <form class="form-inline" action="{{ route("PieceVehicule.store") }}" method="POST" id="form">
                @csrf
                <label class="sr-only" for="piece"></label>
                <div class="input-group mb-2 mr-sm-2">
                  <div class="input-group-prepend">
                    <div class="input-group-text">Piece</div>
                  </div>
                  <input type="text" class="form-control" name="piece" id="SingleInputForm" required>
                </div>
            ...
            <table id="myTable" class="display">
                ...
                <tbody>
                    @foreach ($piece_vehicules as $piece_vehicule)
                        <tr>
                            <td style="text-transform: uppercase;">{{ $piece_vehicule->piece }}</td>
                            <td class="actions d-flex justify-content-end">
                                <form action="{{ route('PieceVehicule.destroy', $piece_vehicule->id) }}" method="POST" style="display: inline-block">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <button type="button" class="btn btn-outline-warning btn-sm" data-toggle="modal" data-target="#exampleModal" onclick="getTableId({{ $piece_vehicule->id }}, '{{ $piece_vehicule->piece }}')"><span class="material-symbols-outlined">edit</span></button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Modifier Piece Vehicule</h5>
              ...
              <div class="modal-body d-flex justify-content-center">
                <form class="form-inline mb-3 mt-3 d-flex justify-content-center" method="POST" id="update-form">
                    @csrf
                    @method("PATCH")
                    <label class="sr-only" for="piece">Piece</label>
                    <div class="input-group mb-2 mr-sm-2">
                      <div class="input-group-prepend">
                        <div class="input-group-text">Piece</div>
                      </div>
                      <input type="text" class="form-control" name="piece" id="UpdateInput" required>
                    </div>
